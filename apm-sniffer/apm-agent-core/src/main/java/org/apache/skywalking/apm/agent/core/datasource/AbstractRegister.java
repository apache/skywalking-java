...
public abstract class AbstractRegister implements DatasourceRegister {
    private static final ILog LOGGER = LogManager.getLogger(AbstractRegister.class);
    ...
    public Database parseDatabase(String connection) {
        ...
        if (database == null && null != connection && !"".equals(connection)) {
            try {
                ...
                LOGGER.error("parse Database url fail ", e);
            }
        }
        return database;
    }
    ...
    protected Integer getIntegerAttribute(ObjectName objectName, String attribute) {
        try {
    ...
    protected Long getLongAttribute(ObjectName objectName, String attribute) {
        try {

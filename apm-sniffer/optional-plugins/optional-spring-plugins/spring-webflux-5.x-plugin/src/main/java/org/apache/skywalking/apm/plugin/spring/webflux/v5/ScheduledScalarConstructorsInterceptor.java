...
public class ScheduledScalarConstructorsInterceptor implements InstanceConstructorInterceptor {
    ...
    public void onConstruct(EnhancedInstance objInst, Object[] allArguments) throws Throwable {
            CoreSubscriber core = null;
            if (allArguments[0] instanceof CoreSubscriber) {
                core = (CoreSubscriber) allArguments[0];
            } else {
                return;
            }
            Context context = core.currentContext();
            Optional<Object> optional = context.getOrEmpty("SKYWALKING_CONTEXT_SNAPSHOT");
            optional.ifPresent(snapshot -> {
                objInst.setSkyWalkingDynamicField(snapshot);
            });
    }
